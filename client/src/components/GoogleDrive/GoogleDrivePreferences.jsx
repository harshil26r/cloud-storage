...
const STORAGE_OPTIONS = [
  {
    ...
    icon: CloudIcon,
    accent: "border-sky-200 bg-sky-50 text-sky-700 dark:border-sky-800 dark:bg-sky-900/20 dark:text-sky-300",
  ...
  },
  {
    ...
    icon: DeviceIcon,
    accent: "border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-800 dark:bg-emerald-900/20 dark:text-emerald-300",
...
export default function GoogleDrivePreferences() {
  ...
  if (loading) {
    return (
      <div className="animate-pulse space-y-3">
        ...
        <div className="h-20 rounded-xl bg-slate-100 dark:bg-gray-800" />
        <div className="h-20 rounded-xl bg-slate-100 dark:bg-gray-800" />
      </div>
    );
  }
  ...
  return (
    <section className="space-y-4">
      <div>
        <h3 className="text-sm font-semibold text-slate-900 dark:text-gray-100">Storage mode</h3>
          ...
          return (
            <button
              ...
              className={`w-full rounded-xl border p-3.5 text-left transition-all ${
                isSelected
                  ? `border-transparent bg-white ring-2 ${option.selectedRing} shadow-sm dark:bg-gray-800`
                  : "border-slate-200 bg-white hover:border-slate-300 hover:shadow-sm dark:border-gray-700 dark:bg-gray-900 dark:hover:border-gray-600"
              } disabled:opacity-60`}
            >
      ...
      <div className="rounded-xl bg-slate-50 px-4 py-3 ring-1 ring-slate-200 dark:bg-gray-800 dark:ring-gray-700">
        <dl className="space-y-2 text-xs">
          <div className="flex justify-between gap-4">
            <dt className="text-slate-500 dark:text-gray-400">Root folder</dt>
            <dd className="font-medium text-slate-800 text-right dark:text-gray-200">
              {preferences.rootGoogleDriveName || "Google Drive"}
            </dd>
          </div>
          <div className="flex justify-between gap-4">
            <dt className="text-slate-500 dark:text-gray-400">Last sync</dt>
            <dd className="font-medium text-slate-800 text-right dark:text-gray-200">
